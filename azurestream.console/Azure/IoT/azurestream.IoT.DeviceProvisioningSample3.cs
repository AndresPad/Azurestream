...
{
    //--------------------------------------------------------------------------------------------------------------
    public class DeviceProvisioningSample3
    {
        ...
        internal static async Task<int> ExecuteAsync(string[] args)
        {
            ...
            var test1 = System.Environment.GetEnvironmentVariable("IotHubConnectionStringEnvVar");
        ...
        private static async Task BulkCreateDevicesAsync(List<Device> devices, RegistryManager registryManager, DeviceProvisionStats stats)
        {
            try
            {
                var bulkResult = await registryManager.AddDevices2Async((IEnumerable<Microsoft.Azure.Devices.Device>)devices);
