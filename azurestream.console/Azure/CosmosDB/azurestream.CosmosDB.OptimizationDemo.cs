...
namespace azurestream.console
{
    ...
    public class CosmosDBOptimizationDemo
    {
        ...
        internal static async Task Execute()
        {
            try
            {
                CosmosDBOptimizationDemo p = new CosmosDBOptimizationDemo();
                await p.Go();
            }
        ...
        public async Task Setup_CosmosResources()
        {
            ...
            using (WebClient wc = new WebClient())
            {
                ...
                for (var i = 0; i < userProductReviews.Count; i++)
                {
                    ...
                    {
                        apa.BOL.CosmosDB.User user = JsonConvert.DeserializeObject<apa.BOL.CosmosDB.User>(userProductReviews[i].ToString());
