<#
 ...
 .DESCRIPTION
       ...
       1. When you login to Azure using this script you can either login programmatically using a Service Principal or interactively using your Username/Password.
        ...
        .\importADFtoSynapseTool.ps1 -sourceADFResourceId /subscriptions/YOURSUBSCRIPTIONID/resourceGroups/azu-data-rg/providers/Microsoft.DataFactory/factories/airstream -destSynapseResourceId /subscriptions/YOURSUBSCRIPTIONID/resourceGroups/AZU-DW-RG/providers/Microsoft.Synapse/workspaces/air

        2. Once you move the pipelines and datasets make sure that if your synapse is git enabled your linked services need to be re-established. The Credentials will not be transfered but this is normal behavior.
...
function MigrateADFSynapse {
    ...
    elseif (-NOT [string]::IsNullOrEmpty($sourceADFResourceId) -and -NOT [string]::IsNullOrEmpty($destSynapseResourceId)) {
        ...
        Write-Host "    $sourceADFResourceId" -ForegroundColor Yellow
        Write-Host "    $destSynapseResourceId" -ForegroundColor Yellow
